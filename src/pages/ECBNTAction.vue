<template>
  <div class="action-page-bg">
    <h1 class="screen-header">Alert - Potential Victim Account - Suspect Beneficiaries added, yet to transfer</h1>
      ...
      <section class="card-section">
        ...
        <div class="table-container">
          <table class="details-table">
            <thead>
              <tr><th>Date</th><th>Time</th><th>Beneficiary</th><th>Mode</th><th>Txn. Ref #</th><th>Txn Description</th></tr>
            </thead>
            <tbody>
...
const beneficiaryDetails = ref([]); // State for the new section
...
const confirmedVictim = ref(''); // Note: a different name from the 'mule' version
...
onMounted(async () => {
  ...
  try {
    ...
    if (combinedDataRes.status === 'fulfilled' && combinedDataRes.value.data) {
      ...
      riskEntity.i4c = { name: i4cData.customer_name, mobileNumber: i4cData.mobile, email: i4cData.email, ifscCode: i4cData.ifsc };
      riskEntity.bank = { name: `${customerDetails.fname || ''} ${customerDetails.mname || ''} ${customerDetails.lname || ''}`.trim(), mobileNumber: customerDetails.mobile, email: customerDetails.email, ifscCode: customerDetails.ifsc, customerId: customerDetails.cust_id, acStatus: data.account_details?.acc_status, aqb: data.account_details?.aqb, availBal: data.account_details?.availBal, productCode: data.account_details?.productCode, relValue: data.account_details?.rel_value, mobVintage: data.account_details?.mob, addl1: data.account_details?.addl1, addl2: data.account_details?.addl2, addl4: data.account_details?.addl4 };
      beneficiaryDetails.value = data.beneficiary_details || [];
    } else {
