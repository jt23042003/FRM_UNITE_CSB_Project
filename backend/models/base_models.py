...
# >>>>>>> 7c36ab16767ee85c938d541a314849fdcadb65d0
...
class ECBCaseData(BaseModel):
    ...
    remarks: Optional[str] = None
    location: Optional[str] = None # Location from original data entry
    disputedAmount: Optional[float] = None # Disputed amount from original data entry
    # Add any other fields needed for case_main

# NEW MODEL: For saving case action data (versioned)
class CaseActionDataSaveRequest(BaseModel):
    case_id: int
    case_type: str
    action_data: Any  # This will be the full action object from the frontend (dict)
    created_by: Optional[str] = None

class CaseActionDataResponse(BaseModel):
    id: int
    case_id: int
    case_type: str
    version: int
    ...
    created_at: datetime
    updated_at: datetime
# NEW MODEL: DepartmentResponse
class DepartmentResponse(BaseModel):
    id: str
    name: str
# NEW MODEL: DepartmentNameResponse for just the name
class DepartmentNameResponse(BaseModel):
    name: str
# NEW MODEL: ActionLogData for the inner data structure
class ActionLogData(BaseModel):
    checked_documents: List[str]
    proof_of_upload_ref: str
    screenshot_filename: Optional[str] = None # Can be Optional as your sample has a filepath, not just filename
    confirmation_action_status: str
    submitted_by: str
    submitted_at: datetime # Use datetime for ISO 8601 format

# NEW MODEL: CaseActionLogResponse for the overall API response
class CaseActionLogResponse(BaseModel):
    success: bool
    data: ActionLogData
# =======
# >>>>>>> 7c36ab16767ee85c938d541a314849fdcadb65d0
