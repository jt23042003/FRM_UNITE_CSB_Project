...
# NEW LIGHTWEIGHT API ENDPOINT: Get User Profile (for case detail pages)
@router.get("/api/user/profile", tags=["User Management"])
async def get_user_profile(
    logged_in_username: Annotated[str, Depends(get_current_username)],
    matcher: Annotated[CaseEntryMatcher, Depends(get_case_matcher_instance)]
) -> Dict[str, Any]:
    """
    Lightweight endpoint to get user profile information.
    Returns only user_type and username - much faster than /api/new-case-list.
    
    This replaces the heavy /api/new-case-list calls in case detail pages
    that were only used to get user role information.
    """
    try:
        # Get user type from the matcher (lightweight database query)
        ...
        print(f"ERROR in get_user_profile: {e}", flush=True)
        traceback.print_exc()
