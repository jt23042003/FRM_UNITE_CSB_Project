...
from fastapi import APIRouter, HTTPException, Query, Depends, Request, Body # Body is likely needed for update_data
...
from models.base_models import CaseMainUpdateData # Ensure this Pydantic model is imported
...
async def update_case_main_api(
    case_id: int,
    update_data: CaseMainUpdateData, # FIX: Use Pydantic model for input data, not raw Dict[str, Any]
    ...
    """
    try:
        # Pass the Pydantic model's dict representation
        updated_case = await matcher.update_case_main_data(case_id, update_data.dict(exclude_unset=True)) # exclude_unset to update only provided fields
        if not updated_case:
    ...
    except CaseNotFoundError as e: # Catch specific CaseNotFoundError from matcher
